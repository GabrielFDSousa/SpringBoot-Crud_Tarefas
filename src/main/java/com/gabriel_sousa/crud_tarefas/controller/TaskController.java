...
public class TaskController {
    ...
    @GetMapping()
    public ResponseEntity<Page<GetTaskResponseDTO>> getPaginatedTasksByUser(@PageableDefault(size = 20) Pageable pageable,
                                                                 @RequestAttribute(RequestAttributeKey.USER_ID) Long userId){
        return ResponseEntity.ok(taskService.getPaginatedTasksByUser(userId, pageable));
    }
